...
//* Ornek1: kisiler dizisindeki joblari tek tek yazdırın
kisiler.forEach((kisi) => console.log(kisi.job));

//* Ornek1: yasları bir artırarak yeni bir diziye saklayınız
...
const raise = maaslar
  .filter(
    (underAvg) =>
      underAvg < maaslar.reduce((acc, avg) => acc + avg / maaslar.length, 0)
  )
  .map((raise20) => raise20 * 1.2);

console.log(raise);

//* Örnek-4: Yasi(age) 33'ten kücük olan kisilerin adlarini (name) listeleyiniz.

const soru4 = kisiler.filter((x) => x.age < 33).map((x) => x.name);
console.log(soru4);
...
//   .map((kisi) => ({ name: kisi.name, age: kisi.age }));
// console.log(dev);
